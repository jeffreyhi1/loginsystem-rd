...
function checkToken() {
	...
	******************************************************************************************/
	...
	If ($oldToken===$testToken) {
		...
		If ($diff<=300) { // Five minutes max
	    	If ($_SESSION["usecookie"]) {
			    If ($_COOKIE["token"]===$oldToken) {
					...
					session_destroy();
					header("Location: http://www.rodsdot.com/oops.asp?p=" . $page . "&t=ec");
				}
			}else{	
	  			return True;
	  		}	
			...
			session_destroy();
			header("Location: http://www.rodsdot.com/oops.asp?p=" . $page . "&t=et");
		}
	}else{
		$_SESSION = array();
		if (isset($_COOKIE[session_name()])) {
		...
		session_destroy();
		header("Location: http://www.rodsdot.com/oops.asp?p=" . $page . "&t=e");
	}
}
