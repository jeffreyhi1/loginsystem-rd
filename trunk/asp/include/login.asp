...
If LCase(Request.ServerVariables("HTTP_METHOD")) = "get" Then
	If lg_debug Then dbMsg = dbMsg & "METHOD=GET<br />" & vbLF
	'*******************************************************************************************************************
	...
	End If
	If lg_debug Then dbMsg = dbMsg & "destination = "& destination &"<br />" & vbLF
	'*******************************************************************************************************************
	'* Determine if we have Session Login
	'*******************************************************************************************************************
	If lg_debug Then dbMsg = dbMsg & "Session login = "& Session("login") &"<br />" & vbLF
	If Session("login") Then
		If lg_debug Then dbMsg = dbMsg & "Request.Cookies user = "& Request.Cookies("user") &"<br />" & vbLF
		If lg_debug Then dbMsg = dbMsg & "Session antiFixation = "& Session("antiFixation") &"<br />" & vbLF
	...
	'*******************************************************************************************************************
	If lg_debug Then dbMsg = dbMsg & "Request.Cookies login= "& Session("login") &"<br />" & vbLF
	If Request.Cookies("login") = "" Then
		'*******************************************************************************************************************
		'* There is no login cookie. Make sure IP is not locked out by loginAttempts
		'*******************************************************************************************************************
		If lg_debug Then dbMsg = dbMsg & "Check for IP ban by loginAttempts<br />" & vbLF
		...
		If Not(db_rs.bof AND db_rs.eof) Then
			...
			If lg_debug Then dbMsg = dbMsg & "Locked = "& laLocked &"<br />" & vbLF
			closeRS
			closeCommand
		End If
		'*******************************************************************************************************************
		...
		'*******************************************************************************************************************
		If lg_debug Then dbMsg = dbMsg & "Setup Login Attempts Record<br />" & vbLF
		If lg_database="access" Then
		...
		execCmd cmdTxt
		If lg_debug Then dbMsg = dbMsg & "Number of Records Affected = "& numAffected &"<br />" & vbLF
		closeCommand
		
		'*******************************************************************************************************************
		'* NOT LOGGED IN: If SSL required and not using SSL, redirect to https
		'*******************************************************************************************************************
		If lg_debug Then dbMsg = dbMsg & "lg_useSSL = "& lg_useSSL &"<br />" & vbLF
		If lg_debug Then dbMsg = dbMsg & "SERVER PORT SECURE = "& Request.ServerVariables("SERVER_PORT_SECURE") &"<br />" & vbLF
	...
	Else
		...
		'*******************************************************************************************************************
		If lg_debug Then dbMsg = dbMsg & "Request.Cookies login = "& Request.Cookies("login") &"<br />" & vbLF
		...
		'*******************************************************************************************************************
		If lg_debug Then dbMsg = dbMsg & "Check if userid or IP is locked out by loginAttempts<br />" & vbLF
		If lg_database="access" Then
		...
		'*******************************************************************************************************************
		If lg_debug Then dbMsg = dbMsg & "Lookup username<br />" & vbLF
		If lg_database="access" Then
		...
		If Not(db_rs.bof AND db_rs.eof) Then
			name = db_rs("name")
			locked = db_rs("locked")
			If lg_debug Then dbMsg = dbMsg & "name = "& name &"<br />" & vbLF
			If lg_debug Then dbMsg = dbMsg & "locked = "& locked &"<br />" & vbLF
		End If
		closeRS
		closeCommand
		...
		Else
			...
			If lg_debug Then dbMsg = dbMsg & "Session login = "& Session("login") &"<br />" & vbLF
			If lg_debug Then dbMsg = dbMsg & "Session name = "& Session("name") &"<br />" & vbLF
			If lg_debug Then dbMsg = dbMsg & "Session userid = "& Session("userid") &"<br />" & vbLF
			
			'*******************************************************************************************************************
			'* Credentials are valid, issue a anti Session Fixation cookie
			'*******************************************************************************************************************
			If lg_debug Then dbMsg = dbMsg & "Credentials are valid - issue anti Session Fixation cookie.<br />" & vbLF
			tmpStr = getGUID
			If lg_debug Then dbMsg = dbMsg & "GUID = "& tmpStr &"<br />" & vbLF
			tmpStr = HashEncode(Session.SessionID & Request.ServerVariables("REMOTE_ADDR") & tmpStr)
			If lg_debug Then dbMsg = dbMsg & "anti Session Fixation Token = "& tmpStr &"<br />" & vbLF
			...
			If lg_log_logins Then
				If lg_debug Then dbMsg = dbMsg & "log_logins = "& log_logins &"<br />" & vbLF
				If lg_database="access" Then
				...
				execCmd cmdTxt
				If lg_debug Then dbMsg = dbMsg & "Log Login Number Affected = "& numAffected &"<br />" & vbLF
				closeCommand	
			End If
...
Else
	...
	'*******************************************************************************************************************
	If lg_debug Then dbMsg = dbMsg & "METHOD=POST, checkToken<br />" & vbLF
	checkToken
	...
	If lg_debug Then dbMsg = dbMsg & "POSTED password = "& password &"<br />" & vbLF
	If lg_debug Then dbMsg = dbMsg & "POSTED userid = "& userid &"<br />" & vbLF
	If lg_debug Then dbMsg = dbMsg & "POSTED remember = "& remember &"<br />" & vbLF
	If lg_debug Then dbMsg = dbMsg & "POSTED destination = "& destination &"<br />" & vbLF
	If userid & ""="" Then
	...
	Else
		...
		If lg_debug Then dbMsg = dbMsg & "useridValue = "& useridValue &"<br />" & vbLF	
	End If
	If password & ""="" Then
	...
	If message="" Then
		...
		'*******************************************************************************************************************
		If lg_debug Then dbMsg = dbMsg & "All required fields exist, attempt to authenticate.<br />" & vbLF
		If lg_database="access" Then
		...
		If Not(db_rs.bof AND db_rs.eof) Then
			...
			If lg_debug Then dbMsg = dbMsg & "db passhash = "& passhash &"<br />" & vbLF
			If lg_debug Then dbMsg = dbMsg & "db name = "& name &"<br />" & vbLF
			If lg_debug Then dbMsg = dbMsg & "db locked = "& locked &"<br />" & vbLF
		End If
		closeRS
		closeCommand
		If locked<>"1" Then
			If lg_debug Then dbMsg = dbMsg & "Calculated passhash = "& HashEncode(password & userid) &"<br />" & vbLF
			If passhash=HashEncode(password & userid) AND NOT(locked="1") Then
				...
				'*******************************************************************************************************************
				If lg_debug Then dbMsg = dbMsg & "term_remember = "& lg_term_remember &"<br />" & vbLF
				If lg_debug Then dbMsg = dbMsg & "user wishes to remember login = "& remember &"<br />" & vbLF
				...
				End If
				If lg_debug Then dbMsg = dbMsg & "Login Cookie Expires: "& Dateadd("yyyy",5,Now()) &"<br />" & vbLF
				...
				'*******************************************************************************************************************
				If lg_debug Then dbMsg = dbMsg & "Clear the Login from attemptedLogins table<br />" & vbLF
				If lg_database="access" Then
				...
				execCmd cmdTxt
				If lg_debug Then dbMsg = dbMsg & "Clear loginAttempts Number Affected = "& numAffected &"<br />" & vbLF
				closeCommand
				
				'*******************************************************************************************************************
				'* Clear the form token
				'*******************************************************************************************************************
				If lg_debug Then dbMsg = dbMsg & "Clear the form token cookie.<br />" & vbLF
				...
				'* Logged in, redirect
				'*******************************************************************************************************************
				If lg_debug Then dbMsg = dbMsg & "Valid login, redirect<br />" & vbLF
				Response.Redirect(destination)
			Else
				If lg_debug Then dbMsg = dbMsg & "Bad Login: increment attemptedLogins record.<br />" & vbLF
				...
				Else	
					cmdTxt = "SELECT id, loginAttemptUserID, loginAttemptNumber, loginAttemptDate, loginAttemptIP, loginAttemptLocked FROM loginAttempts WHERE (loginAttemptUserID = ?) OR (loginAttemptIP = ?);"
				End If
				If lg_debug Then dbMsg = dbMsg & "First look-up the attemptedLogin record<br />" & vbLF
				...
				If Not(db_rs.bof AND db_rs.eof) Then
					...
					laLocked = db_rs("loginAttemptLocked")
					If lg_debug Then dbMsg = dbMsg & "db ID = "& laID &"<br />" & vbLF
					If lg_debug Then dbMsg = dbMsg & "db UserID = "& laUserID &"<br />" & vbLF
					If lg_debug Then dbMsg = dbMsg & "db Number = "& laNumber &"<br />" & vbLF
					If lg_debug Then dbMsg = dbMsg & "db Date = "& laDate &"<br />" & vbLF
					If lg_debug Then dbMsg = dbMsg & "db IP = "& laIP &"<br />" & vbLF
					If lg_debug Then dbMsg = dbMsg & "db Locked = "& laLocked &"<br />" & vbLF
				End If
				closeRS
				closeCommand
				laNumber = laNumber + 1
				If lg_debug Then dbMsg = dbMsg & "Number incremented to: "& laNumber &"<br />" & vbLF
				If lg_debug Then dbMsg = dbMsg & "Max Logins = "& lg_login_attempts &"<br />" & vbLF
				If (laNumber >= lg_login_attempts) Then
					' Lock the account and redirect to forbidden
					If lg_debug Then dbMsg = dbMsg & "Exceeded max logins, lock the account and ban the IP<br />" & vbLF
					If lg_database="access" Then
					...
					execCmd cmdTxt
					If lg_debug Then dbMsg = dbMsg & "Lock loginAttempts recordsAffected = "& numAffected &"<br />" & vbLF
					closeCommand
					Response.Redirect("http://" & lg_domain & lg_forbidden &"?r=5")
				Else
					' Update the record with the UserID and number of login attempts
					If lg_debug Then dbMsg = dbMsg & "Have not exceeded max login attempts. Update record.<br />" & vbLF
					If lg_database="access" Then
					...
					execCmd cmdTxt
					If lg_debug Then dbMsg = dbMsg & "Updated loginAttempts. numAffected = "& numAffected &"<br />" & vbLF
					closeCommand				
				End If	
				message = lg_phrase_login_error
				If lg_debug Then dbMsg = dbMsg & "message = "& message &" 1 <br />" & vbLF
			End if
		Else
			message = lg_phrase_login_error
			If lg_debug Then dbMsg = dbMsg & "message = "& message &" 2 <br />" & vbLF
		End if	
	End If
End if
%>